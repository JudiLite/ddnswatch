...
def main_keyboard():
    return [[{"text":"➕ 添加监测","callback_data":"add"},{"text":"📋 监测列表","callback_data":"list"}],
            ...
            [{"text":"✖️ 关闭菜单","callback_data":"close"}]]
...
def callback(q):
    ...
    elif data=="menu": SESSIONS.pop(uid,None); menu(chat,mid=mid)
...
def run():
    ...
    api("setMyCommands",{"commands":[{"command":"start","description":"打开主菜单"},{"command":"menu","description":"打开主菜单"},{"command":"close","description":"关闭当前菜单"},{"command":"cancel","description":"取消当前操作"}]})
